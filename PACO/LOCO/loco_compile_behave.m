...
ssids = [001, 002, 003, 004, 005, 010, 017];
...
for s = 1:length(ssids)
    ...
    filename = sprintf('/Volumes/bamlab/Experiments/PACO/LOCO/data/%s/sub_%s_PACOLOCO_test.csv', subj, subj);
    delimiter = ',';
    startRow = 2;
    ...
    for col=[1,2,3,4,6,7,8,9,12,13,14,15,17,18,19,20,24,28]
    ...
    rawNumericColumns = raw(:, [1,2,3,4,6,7,8,9,12,13,14,15,17,18,19,20,24,28]);
    rawStringColumns = string(raw(:, [5,10,11,16,21,22,23,25,26,27]));
    ...
    StimuliName1 = cell2mat(rawNumericColumns(:, 1));
    ...
    User_Color = cell2mat(rawNumericColumns(:, 7));
    User_Rad = cell2mat(rawNumericColumns(:, 8));
    User_AproxColor = categorical(rawStringColumns(:, 2));
    Correct_OldOrNew = categorical(rawStringColumns(:, 3));
    Correct_Image_x = cell2mat(rawNumericColumns(:, 9));
    Correct_Image_y = cell2mat(rawNumericColumns(:, 10));
    Correct_Color = cell2mat(rawNumericColumns(:, 11));
    Correct_Rad = cell2mat(rawNumericColumns(:, 12));
    Correct_AproxColor = categorical(rawStringColumns(:, 4));
    locationDist = cell2mat(rawNumericColumns(:, 13));
    radColorDist = cell2mat(rawNumericColumns(:, 14));
    OldOrNew_RT = cell2mat(rawNumericColumns(:, 15));
    Test_RT = cell2mat(rawNumericColumns(:, 16));
    ...
    participant = cell2mat(rawNumericColumns(:, 17));
    date1 = categorical(rawStringColumns(:, 8));
    expName = categorical(rawStringColumns(:, 9));
    psychopyVersion = categorical(rawStringColumns(:, 10));
    frameRate = cell2mat(rawNumericColumns(:, 18));
...
recognition_headers = {'OverallAcc', 'HR', 'FA', 'CHR', 'O_RT', 'N_RT', 'Mean_CDA', 'Mean_CDC', 'Median_CDA', 'Median_CDC', 'Mean_LDA', 'Mean_LDC', 'Median_LDA', 'Median_LDC'};
color_headers = {};
location_headers = {};
...
out_data = [ssids', recognition_data, color_data, location_data];
...
compiledBehave = sprintf('%s/Analysis/loco_compiledBehave.txt', pwd);
...
% Wrtie .mat file
save(sprintf('%s/loco_compiledBehave.mat',subDir), 'out_data');
